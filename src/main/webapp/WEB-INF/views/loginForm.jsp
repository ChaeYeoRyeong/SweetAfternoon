...
<form action="<c:url value="/login/login"/>" method="post" onsubmit="return formCheck(this);">
	...
	<input type="text" name="user_id" value="${cookie.user_id.value}" placeholder="아이디" autofocus>
	<input type="password" name="user_pwd" placeholder="비밀번호">
	<input type="hidden" name="toURL" value="${param.toURL}">
	<button>로그인</button>
	<div>
		<label><input type="checkbox" name="rememberId" value="on" ${empty cookie.user_id.value ? "":"checked"}> 아이디 기억</label> |
	...
	<script>
		function formCheck(frm) {
			...
			if(frm.user_id.value.length==0) {
				setMessage('아이디를 입력해주세요.', frm.user_id);
				return false;
			}
